 ...
 * 
 * 1.0.0 (21.09.2014)
 *   - Initial version for Xiaomi Redmi 1S (armani) a port by Tarun93
 * 
 */
...
// original wcd9306 alsa driver
/*****************************************/

void boeffla_sound_hook_tapan_codec_probe(struct snd_soc_codec *codec_pointer)
{
...
unsigned int boeffla_sound_hook_tapan_write(unsigned int reg, unsigned int value)
{
	...
	switch (reg)
	{
		case TAPAN_A_CDC_RX1_VOL_CTL_B2_CTL: // headphone L
		{
			value = headphone_volume_l;
			break;
		}

		case TAPAN_A_CDC_RX2_VOL_CTL_B2_CTL: // headphone R
		{
			value = headphone_volume_l;
			break;
		}

		case TAPAN_A_CDC_RX3_VOL_CTL_B2_CTL: // speaker
		{
			value = speaker_volume_l;
			break;
		}

		case TAPAN_A_CDC_RX4_VOL_CTL_B2_CTL: // speaker
		{
			value = speaker_volume_r;
			break;
		}

//		case TAPAN_A_CDC_TX3_VOL_CTL_GAIN: // mic level general
//		{
//			value = mic_level;
//			break;
//		}
...
static void reset_audio_hub(void)
{
	...
	tapan_write_no_hook(codec, TAPAN_A_CDC_RX1_VOL_CTL_B2_CTL, HEADPHONE_DEFAULT + HEADPHONE_REG_OFFSET);
	tapan_write_no_hook(codec, TAPAN_A_CDC_RX2_VOL_CTL_B2_CTL, HEADPHONE_DEFAULT + HEADPHONE_REG_OFFSET);

	tapan_write_no_hook(codec, TAPAN_A_CDC_RX3_VOL_CTL_B2_CTL, SPEAKER_DEFAULT + SPEAKER_REG_OFFSET);
	tapan_write_no_hook(codec, TAPAN_A_CDC_RX4_VOL_CTL_B2_CTL, SPEAKER_DEFAULT + SPEAKER_REG_OFFSET);

//	tapan_write_no_hook(codec, TAPAN_A_CDC_TX3_VOL_CTL_GAIN, MICLEVEL_DEFAULT + MICLEVEL_REG_OFFSET);

	if (debug)
		printk("Boeffla-sound: wcd9306 audio hub reset done\n");
}
					...
					const char *buf, size_t count)
{
	...
	// set new values
	tapan_write_no_hook(codec, TAPAN_A_CDC_RX1_VOL_CTL_B2_CTL, 
		headphone_volume_l + HEADPHONE_REG_OFFSET);
	tapan_write_no_hook(codec, TAPAN_A_CDC_RX2_VOL_CTL_B2_CTL, 
					...
					const char *buf, size_t count)
{
	...
	// set new values
	tapan_write_no_hook(codec, TAPAN_A_CDC_RX3_VOL_CTL_B2_CTL, 
		speaker_volume_l + SPEAKER_REG_OFFSET);
	tapan_write_no_hook(codec, TAPAN_A_CDC_RX4_VOL_CTL_B2_CTL, 
					...
					const char *buf, size_t count)
{
	...
	// set new value
	tapan_write_no_hook(codec, TAPAN_A_CDC_TX3_VOL_CTL_GAIN, 
...
static ssize_t register_dump_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX1_VOL_CTL_B1_CTL: %d\n", 
		tapan_read(codec, TAPAN_A_CDC_RX1_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX2_VOL_CTL_B1_CTL: %d\n", 
		tapan_read(codec, TAPAN_A_CDC_RX2_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX3_VOL_CTL_B1_CTL: %d\n", 
		tapan_read(codec, TAPAN_A_CDC_RX3_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX4_VOL_CTL_B1_CTL: %d\n", 
		tapan_read(codec, TAPAN_A_CDC_RX4_VOL_CTL_B1_CTL));

	// Output volumes Bank2
	sprintf(buf+strlen(buf), "\n** Output values bank 2\n");

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX1_VOL_CTL_B2_CTL: %d (head l)\n", 
		tapan_read(codec, TAPAN_A_CDC_RX1_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX2_VOL_CTL_B2_CTL: %d (head r)\n", 
		tapan_read(codec, TAPAN_A_CDC_RX2_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX3_VOL_CTL_B2_CTL: %d (speaker)\n", 
		tapan_read(codec, TAPAN_A_CDC_RX3_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_RX4_VOL_CTL_B2_CTL: %d (speaker)\n", 
		tapan_read(codec, TAPAN_A_CDC_RX4_VOL_CTL_B2_CTL));

	// Input gains
	sprintf(buf+strlen(buf), "\n** Input gains\n");

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_TX1_VOL_CTL_GAIN: %d\n", 
		tapan_read(codec, TAPAN_A_CDC_TX1_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_TX2_VOL_CTL_GAIN: %d\n", 
		tapan_read(codec, TAPAN_A_CDC_TX2_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_TX3_VOL_CTL_GAIN: %d\n", 
		tapan_read(codec, TAPAN_A_CDC_TX3_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TAPAN_A_CDC_TX4_VOL_CTL_GAIN: %d (mic)\n", 
		tapan_read(codec, TAPAN_A_CDC_TX4_VOL_CTL_GAIN));
